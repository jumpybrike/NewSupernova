...
**Level 0 (Root):** `/` (Homepage). **Level 1 (Primary Navigation):** `/discover` (Browse hub), `/library` (Product catalog), `/articles` (Editorial content), `/membership` (Subscription tiers), `/about` (Mission/contact). **Level 2 (Discover Taxonomy):** `/discover/era/{decade}` (1920s, 1930s, 1940s, 1950s, 1960s, 1970s, 1980s), `/discover/theme/{theme-slug}` (30+ themes: time-travel, space-opera, dystopia, first-contact, robots-ai, etc.), `/discover/author/{author-slug}` (200+ authors alphabetically), `/discover/format/{format-type}` (novels, novellas, short-stories, collections, radio-dramas). **Level 2 (Library Categories):** `/library/ebooks`, `/library/audiobooks`, `/library/collections`, `/library/bundles`, `/library/new-releases`, `/library/enhanced-editions`. **Level 2 (Articles Categories):** `/articles/reviews`, `/articles/authors` (author profiles/biographies), `/articles/guides` (reading lists, beginner guides), `/articles/history` (genre evolution, cultural context), `/articles/interviews`. **Level 3 (Product Details):** `/library/{format}/{product-slug}` (e.g., `/library/ebooks/foundation-asimov`, `/library/audiobooks/left-hand-darkness-leguin`), `/library/collections/{collection-slug}` (e.g., `/library/collections/essential-golden-age`, `/library/collections/complete-asimov`). **Level 3 (Article Details):** `/articles/{category}/{article-slug}` (e.g., `/articles/reviews/dune-review`, `/articles/authors/ursula-le-guin-biography`). **Level 3 (Author Pages):** `/discover/author/{author-slug}` (individual author landing pages with biography, complete works, themes, influences). **Utility Pages:** `/search` (search results), `/cart` (shopping cart), `/checkout` (purchase flow), `/account` (member dashboard), `/account/orders` (purchase history), `/account/settings` (profile/billing), `/account/membership` (subscription management), `/login`, `/register`, `/password-reset`. **Legal/Support:** `/about/mission`, `/about/contact`, `/about/faq`, `/about/terms`, `/about/privacy`, `/about/refunds`, `/about/dmca`. **Dynamic URLs:** `/search?q={query}&type={content-type}`, `/discover/{taxonomy}?era={decade}&theme={theme}&format={format}&page={n}` (filtered/paginated browse results). Sitemap totals: ~50 static pages, 500-1000 product pages (Phase 1-3 growth), 200-500 article pages, 200+ author pages, 50+ taxonomy/collection pages = 1,000-2,000 total indexed pages at maturity.

### 12.2 Glossary of Terms

**Breadcrumbs:** Hierarchical navigation trail showing user's location within site structure (Home > Discover > Era > 1950s), enabling quick traversal to parent pages. **Canonical URL:** Preferred URL for content when multiple URLs access same content; prevents duplicate content SEO penalties via rel="canonical" tag. **Conversion Funnel:** Sequential steps users take from discovery to purchase (landing → browse → product view → cart → checkout → purchase); tracking identifies optimization opportunities. **Faceted Navigation:** Multi-dimensional filtering system allowing simultaneous filter application across attributes (era + theme + format) refining large result sets to relevant subsets. **Hero Section:** Prominent above-fold website area (typically homepage) with primary headline, value proposition, and call-to-action driving user engagement. **Information Architecture (IA):** Structural design of shared information environments; defines organization, labeling, navigation, and search systems enabling findability and usability. **Long-Tail Keywords:** Specific, lower-volume search phrases (e.g., "1950s dystopian science fiction audiobooks") with higher conversion intent vs. generic terms; foundation of SF Supernova's SEO strategy. **Mega Menu:** Expansive dropdown navigation panel displaying multiple columns/sections with rich content (images, descriptions) vs. simple link lists; used for complex site hierarchies. **Metadata:** Descriptive information about content (title, author, publication year, themes, format) enabling categorization, search, and relationship mapping across catalog. **Responsive Design:** Web design approach ensuring optimal viewing/interaction across device sizes (mobile/tablet/desktop) through flexible layouts, images, and CSS media queries. **Schema.org Markup:** Structured data vocabulary enabling search engines to understand page content semantics; powers rich snippets (star ratings, prices, breadcrumbs) in search results. **SEO (Search Engine Optimization):** Practice of optimizing website content, structure, and technical implementation to improve organic search rankings and traffic acquisition. **Sitemap:** Hierarchical list or file (XML/HTML) documenting all website pages; aids user navigation and search engine crawling/indexing. **Taxonomy:** Classification system organizing content into hierarchical categories (eras, themes, authors, formats); enables browsing, filtering, and relationship discovery. **URL Slug:** URL-friendly text identifier for specific page (e.g., "foundation-asimov"); typically derived from page title with lowercase, hyphens, no special characters. **User Journey:** Path users take through website achieving goals; maps touchpoints from awareness through consideration to conversion and retention. **WCAG (Web Content Accessibility Guidelines):** International standards ensuring web content accessibility for users with disabilities; SF Supernova targets WCAG 2.1 Level AA compliance.
